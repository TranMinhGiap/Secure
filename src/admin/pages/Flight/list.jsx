...
const FlightList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const result = await GET("/flights", params);
        console.log(result);
        setData(result.data);
        setPagination(prev => ({
          ...prev,
  ...
  const columns = [
    ...
    { 
      ...
      dataIndex: 'scheduled',
      render: (_, record) => (
        <Tag color="purple">{record.scheduled}</Tag>
      )
    },
    { 
      title: 'Xuất phát', 
      ...
      render: (_, record) => (
        <Tag color="purple">{record.departure_airport.name}</Tag>
      )
    },
    { 
      title: 'Đến', 
      ...
      render: (_, record) => (
        <Tag color="purple">{record.arrival_airport.name}</Tag>
      )
    },
    {
      dataIndex: "action",
      width: 70,
      render: (_, record) => (
        <ActionTable id={record["id"]} reload={handleReload} url={`flights`} api={api} />
      )
    }
  ];
