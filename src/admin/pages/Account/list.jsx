...
const AccountList = () => {
    ...
    const fetchData = async () => {
      ...
      if (location.state?.success) {
        api.open({
          ...
          description: "Đã thêm 1 tài khoản mới",
      ...
      try {
        const result = await GET("/api/v1/admin/accounts", params);
        console.log(result);
        setData(result.data);
        setPagination(prev => ({
          ...prev,
  ...
  const columns = [
      ...
      render: (_, record) => (
        <Space>
          <Image
            ...
            src={record.avatar ? record.avatar : "error"}
            fallback={
              (!record.avatar)
    ...
    },
    {
      title: 'Họ và tên',
      dataIndex: 'fullName',
      ...getColumnSearchProps('fullName', 'Tìm theo tên'),
      sorter: true,
    },
    { 
      title: 'Trạng thái', 
      dataIndex: 'status',
      render: (_, record) => (
        <ChangeStatusTable id={record["_id"]} status={record.status} url="accounts"/>
      )
    },
    { 
      ...
      render: (_, record) => (
        <Tag color="purple">
          {record.roleInfo ? record.roleInfo.title : "Chưa có"}
        </Tag>
      )
    },
    {
      ...
      width: 70,
      render: (_, record) => (
        <ActionTable id={record["_id"]} reload={handleReload} url={`accounts`} api={api} />
      )
    }
  ];
          ...
          <Flex justify='space-between' style={{padding: '16px 8px', backgroundColor: "white", borderRadius: "12px 12px 0 0"}}>
            <Space>
              <Select
                ...
                disabled={loading}
              />
              <ChangeMulti ids={selectedRowKeys} reload={handleReload} url="accounts"/>
            </Space>
            <Space>
              <Dropdown
